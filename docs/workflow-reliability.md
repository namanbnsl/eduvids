...
When `gemini-3.8-flash` reports overload/high demand or service unavailability, generation tries `gemini-3.5-flash-lite` once. Both calls share the original 180-second deadline and report health for their own model/key. Credential errors, quota errors, invalid output, and cancellation do not trigger this fallback. If both models fail, the error returns to Upstash for its durable retry. Flash Lite output still passes the existing scene-plan parsing and script validation; model quality can differ. Provider overload is shown as temporary unavailability rather than a script-rendering failure.
...
5. Verify both the primary and fallback model IDs are available to that Google account.
