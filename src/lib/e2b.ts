...
const buildAstValidationCommand = (scriptPath: string) =>
  [
    ...
    "import sys",
    "import importlib",
    "from manim import VMobject as __VMobject",
    `script_path = r"${scriptPath}"`,
    ...
    "        return cached",
    "    try:",
    "        module = importlib.import_module(module_name or \"manim\")",
    "    except Exception as exc:",
    "        module_cache[module_name] = exc",
    "        return exc",
    ...
    "            self.imported_modules[local_name] = module_name",
    "        self.generic_visit(node)",
    "",
    "    def visit_ImportFrom(self, node):",
    ...
    "            self.imported_from[local_name] = (module_name, alias.name)",
    "        self.generic_visit(node)",
    "",
    "    def visit_Call(self, node):",
    ...
    "analyzer.visit(tree)",
    "",
    "def is_locally_defined(name):",
    "    return (",
    "        name in analyzer.class_defs",
    "        or name in analyzer.func_defs",
    "        or name in analyzer.assigned_names",
    "    )",
    "",
    "def iter_manim_star_modules():",
    "    for module_name in analyzer.star_imports:",
    "        if (module_name or '').startswith('manim'):",
    "            yield module_name or 'manim'",
    "",
    "vmobject_issues = []",
    "",
    "def check_attribute_chain(chain):",
    "    if not chain:",
    "        return None",
    "    base = chain[0]",
    "    attr_path = chain[1:]",
    "    module_name = analyzer.imported_modules.get(base)",
    "    if module_name is None:",
    "        if base in analyzer.imported_from:",
    "            module_name, original = analyzer.imported_from[base]",
    "            attr_path = [original] + attr_path",
    "        else:",
    "            return None",
    "    module = load_module(module_name)",
    "    if isinstance(module, Exception):",
    "        return f\"Failed to import '{module_name}' to validate VMobject usage: {module}\"",
    "    obj = module",
    "    for part in attr_path:",
    "        if not hasattr(obj, part):",
    "            return f\"Attribute '{'.'.join(chain)}' not found in module '{module_name}'\"",
    "        obj = getattr(obj, part)",
    "    if not hasattr(obj, '__mro__'):",
    "        return None",
    "    if isinstance(obj, type) and issubclass(obj, __VMobject):",
    "        return None",
    "    return f\"'{'.'.join(chain)}' is not a VMobject.\"",
    "",
    "for call in analyzer.calls:",
    "    target_type, target = resolve_call_target(call)",
    "    if target_type == 'name':",
    "        func_name = target",
    "        if not func_name or not func_name[0].isupper():",
    "            continue",
    "        if is_locally_defined(func_name):",
    "            continue",
    "        module_info = analyzer.imported_from.get(func_name)",
    "        candidate_modules = []",
    "        attribute_name = func_name",
    "        if module_info:",
    "            module_name, original_name = module_info",
    "            if not (module_name or '').startswith('manim'):",
    "                continue",
    "            candidate_modules = [module_name or 'manim']",
    "            attribute_name = original_name",
    "        else:",
    "            candidate_modules = list(iter_manim_star_modules())",
    "            if not candidate_modules and any((module or '').startswith('manim') for module in analyzer.imported_modules.values()):",
    "                candidate_modules = ['manim']",
    ...
    "            module = load_module(module_name)",
    "            if isinstance(module, Exception):",
    "                vmobject_issues.append(f\"Failed to import '{module_name}' to validate VMobject usage: {module}\")",
    "                continue",
    ...
    "                break",
    "        if not found:",
    "            vmobject_issues.append(f\"Unknown VMobject '{func_name}'. Confirm it exists in manim or define it before use.\")",
    "    elif target_type == 'attr':",
    "        issue = check_attribute_chain(target)",
    "        if issue:",
    "            vmobject_issues.append(issue)",
    "",
    "if vmobject_issues:",
    "    for issue in vmobject_issues:",
    "        print(issue, file=sys.stderr)",
    "    raise SystemExit('AST validation failed due to unknown VMobject usage.')",
    "if issues:",
    "    for issue in issues:",
    "        print(issue, file=sys.stderr)",
