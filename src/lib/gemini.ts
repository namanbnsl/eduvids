...
export async function generateThumbnailManimScript({
  ...
  const { text } = await generateText({
    ...
    prompt: [
      ...
      "5. Features the video title prominently with large, bold text",
      "6. Uses WHITE for main text and YELLOW for emphasis",
      "7. Includes 1-2 key visual elements that represent the video topic",
      "8. Has a clean, professional layout with good spacing",
      "9. Uses self.add() to add all objects (NO self.play() calls)",
      "10. Keeps text readable and visually balanced",
      "11. Uses simple shapes and clear hierarchy",
      "",
