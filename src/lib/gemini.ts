...
export async function verifyManimScript({
  ...
  const verifierSystemPrompt = buildAugmentedSystemPrompt(
    [
      ...
      "8. TEXT LAYOUT ERRORS (cause cutoffs and overlaps):",
      "   - Long Text() strings without line breaks will get cut off",
      "   - If text appears longer than ~60 characters, it MUST be split into lines",
      "   - Use Text() with \\n or multiple Text objects in VGroup arranged DOWN",
      "   - Bullet points must use: bullets.to_edge(LEFT, buff=1.0) and arrange(DOWN, aligned_edge=LEFT)",
      "   - Titles must use: title.to_edge(UP, buff=0.5), content at move_to(ORIGIN) or shift(DOWN)",
      "   - NEVER place content and title in same vertical space (minimum 0.8 units spacing)",
      "",
      ...
      "For long text, split into: VGroup(Text('line1'), Text('line2')).arrange(DOWN, buff=0.2)",
      "For bullets, use: bullets.arrange(DOWN, aligned_edge=LEFT).to_edge(LEFT, buff=1.0)",
      "For title+content, use: title.to_edge(UP, buff=0.5), then FadeOut(title) or content.move_to(ORIGIN)",
