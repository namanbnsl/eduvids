...
export async function uploadImage({
  ...
  try {
    ...
    const base64Data = imagePath.replace("data:image/png;base64,", "");
    const buffer = Buffer.from(base64Data, "base64");
    console.log(`Decoded thumbnail base64 data: ${buffer.length} bytes`);

    const fileName = `thumbnail_${userId}_${Date.now()}.png`;
    const file = new UTFile([new Uint8Array(buffer)], fileName, {
      type: "image/png",
    });

    console.log("Starting thumbnail upload to UploadThing...");
    const response = await utapi.uploadFiles([file]);
    ...
    console.log(`Thumbnail uploaded successfully: ${uploadUrl}`);
    return uploadUrl;
  } catch (error) {
    console.error("Thumbnail upload failed:", error);
    throw new Error(`Thumbnail upload failed: ${(error as Error).message}`);
  }
}
